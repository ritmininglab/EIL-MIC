...
class NLL2(layers.Layer):
    def __init__(self, numclass=2,usemask=False,sparselabel=True, div=1, name='name'):
...
class KL(layers.Layer):
    def __init__(self, numclass=2,usemask=False,sparselabel=True,name='name'):
...
def ViT(datas, dims, bnparam, cnn, useevi=False):
    embed_dim = dims[0]
    ff_dim = dims[1]
    nlabel = dims[2]
    sparselabel = dims[3]
    ...
    f5 = Conv2D(embed_dim,3,activation='linear',padding="same",name='fa'+str(i),
                kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(f4)
    f5 = AveragePooling2D(pool_size=(2,2),padding="valid", name='fpool')(f5)
    """
    f5 = Conv2D(embed_dim,3,activation='relu',padding="same",name='fb'+str(i),
                kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(f5)
    """
    f5 = tf.keras.layers.Reshape((-1, embed_dim), name='freshape')(f5)
    f5c = ImgEmbedding(T=25, embed_dim=embed_dim, name='f5c')(f5)
    """
    f5 = Patches(patch_size=14,name='f5')(datas[0])
    f5c = ImgEmbedding(T=16*16, embed_dim=embed_dim, name='f5c')(f5)
    """
    key = Embedding(input_dim=nlabel, output_dim=embed_dim, embeddings_regularizer=L2(1e-5),
                    name='keyembed')(datas[1])
    for i in range(1):
        attn1 = MultiHeadAttention(num_heads=2,key_dim=embed_dim, trainable=trn2,name='attn'+str(i))(query=key, value=f5c, key=f5c)
        add1a = Lambda(lambda x: x[0]+x[1], name='adda'+str(i)) ([key, attn1])
        ln1 = LayerNormalization(trainable=trn2,name='lna'+str(i)) (add1a)
        fc1 = Dense(embed_dim, activation="linear", name='ff'+str(i),
                    kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(ln1)
        add1b = Lambda(lambda x: x[0]+x[1], name='addb'+str(i)) ([ln1, fc1])
        ln2 = LayerNormalization(trainable=trn2,name='lnb'+str(i)) (add1b)
    if not useevi:
        lnow = Dense(1, activation="sigmoid", name='lnow',
                 kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(ln2)
        return lnow
    else:
        lnow = Dense(2, activation="exponential", name='lnow0',
                 kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(ln2)
        nll = NLL2(numclass=2, sparselabel=sparselabel,name='nll')([lnow, datas[2]])
        kl = KL(sparselabel=sparselabel, name='kl')([lnow, datas[2]])
        return [lnow, nll, kl]
...
def ViTevi(datas, dims, bnparam, cnn):
    ...
    for i in range(1):
        fc = Dense(embed_dim, activation="relu", name='fc'+str(i),
                   kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(key)
        fd = Dense(embed_dim, activation="linear", name='fd'+str(i),
                   kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(fc)
        fd = LayerNormalization(name='out'+str(i)) (fd)
    lnow0 = Dense(nlabel*2, activation="exponential", name='lnow0',
                 kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(fd)
    lnow = Lambda(lambda x: tf.reshape(x, [-1,nlabel,2]), name='lnow') (lnow0)
    nll = NLL2(numclass=2, name='nll')([lnow, datas[1]])
    kl = KL(name='kl')([lnow, datas[1]])
    return [lnow, nll, kl]
...
def ViTcnn(datas, dims, bnparam):
    ...
    f5 = Conv2D(64,3,activation='relu',padding="same",name='fa',
                kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(datas[0])
    ...
    return lnow
    """
    lnow0 = Dense(2*nlabel, activation="exponential", name='lnow0')(fd)
    lnow = Lambda(lambda x: tf.reshape(x, [-1,nlabel,2]), name='lnow') (lnow0)
    nll = NLL2(numclass=2, name='nll')([lnow, datas[1]])
    kl = KL(name='kl')([lnow, datas[1]])
    return [lnow, nll, kl]
    """
def ViTevi2022(datas, dims, bnparam, cnn):
    embed_dim = dims[0]
    ff_dim = dims[1]
    nlabel = dims[2]
    trn1 = bnparam[1][0]
...
As long as you do not use custom layer
"""
...
class Decoder(Layer):
    ...
    def call(self, inputs, encoder_outputs, unpair=None, mask=None):
        ...
        if unpair is not None:
            padding_mask = tf.math.multiply(unpair, padding_mask)
...
def ViTcaptionVgg(datas, dims, bnparam, cnn):
    ...
    for i in range(1):
        fd = TimeDistributed(Dense(embed_dim, activation="relu",
                                   kernel_regularizer=L2(1e-5),), name='finalc'+str(i))(seq)
    lnow = TimeDistributed(Dense(vocsize, activation="softmax",
                                 kernel_regularizer=L2(1e-5),), name='lnow')(fd)
    return lnow
def ViTcaptionVgg2(datas, dims, bnparam, cnn):
    ...
    f5 = Conv2D(embed_dim,3,activation='relu',padding="same",name='fb'+str(i),
                kernel_regularizer=L2(1e-5),bias_regularizer=L2(1e-5),)(f5)
    f5 = Reshape((-1, embed_dim), name='freshape')(f5)
    f5c = ImgEmbedding(T=25, embed_dim=embed_dim, name='f5c')(f5)
    """
    seq = SeqEmbedding(T=Tcaption, V=vocsize, embed_dim=embed_dim, name='embedcap')(datas[1])
...
def ViTcaptionVgg2evi(datas, dims, bnparam, cnn):
    embed_dim = dims[0]
    ff_dim = dims[1]
    keysize = dims[2]
    vocsize = dims[3]
    ...
    keymask = Lambda(lambda x: x>0, name='keymask')(datas[2])
    for i in range(2):
        seq = Decoder(embed_dim, num_heads=2, name='decoderl'+str(i))(seq, f5c, None)
        seq = DecoderKey(embed_dim, num_heads=2, name='decoderk'+str(i))(seq, key, keymask)
    
    for i in range(1):
        fd = TimeDistributed(Dense(embed_dim, activation="relu",
                                   kernel_regularizer=L2(1e-5),), name='finalc'+str(i))(seq)
    lnow = TimeDistributed(Dense(vocsize, activation="exponential",
                                 kernel_regularizer=L2(1e-5),), name='lnow')(fd)
    nll = NLL2(numclass=vocsize, sparselabel=sparselabel,name='nll')([lnow, datas[3]])
    kl = KL(numclass=vocsize, sparselabel=sparselabel,name='kl')([lnow, datas[3]])
    return [lnow, nll, kl]


def ViTcaptionVgg2unpair(datas, dims, bnparam, cnn):
    embed_dim = dims[0]
    ff_dim = dims[1]
    keysize = dims[2]
    vocsize = dims[3]
    ...
    keymask = Lambda(lambda x: x>0, name='keymask')(datas[2])
    for i in range(2):
        seq = Decoder(embed_dim, num_heads=2, name='decoderl'+str(i))(seq, f5c, datas[4]) 
        if dropout is not None:
            seq = Dropout(dropout, name='dropa'+str(i))(seq)
        seq = DecoderKey(embed_dim, num_heads=2, name='decoderk'+str(i))(seq, key, keymask)
        if dropout is not None:
            seq = Dropout(dropout, name='dropb'+str(i))(seq)
...
def ViTcaptionSoftmax(datas, dims, bnparam, cnn):
    embed_dim = dims[0]
    ff_dim = dims[1]
    keysize = dims[2]
    vocsize = dims[3]
